<?php
/**
 * Leaffox 系统首页模版 - Default（内置）
 * ============================================
 * 模版类型：landing（系统落地页）
 * 文件路径：templates/landing/default.php
 * 变量说明：
 *   $siteName   (string)  站点名称
 *   $siteDesc   (string)  站点描述
 *   $baseUrl    (string)  站点根 URL
 *   $isLogin    (bool)    当前访问者是否已登录
 *   $settings   (array)   全站设置（键值对）
 *     - powered_by_enabled   是否显示"由XX提供"页脚
 *     - announcement         公告内容
 *     - reg_enabled          是否开放注册
 *     - show_free_make_btn   是否显示"免费制作"按钮
 *     - footer_html          自定义页脚 HTML
 *     - stat_total_users     注册用户总数
 *     - stat_total_links     链接总数
 *     - stat_total_views     总访问量
 * 返回值格式：
 *   直接输出 HTML，无需 return
 *   模版变量在 <?php ?> 中直接可用
 * ============================================
...
.navbar{display:flex;align-items:center;justify-content:space-between;padding:18px 0;border-bottom:1px solid rgba(255,255,255,0.06)}
.nav-logo{display:flex;align-items:center;gap:10px;font-size:20px;font-weight:800;color:#fff;text-decoration:none}
.nav-logo .logo-icon{width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#6366f1,#a78bfa);display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:900;color:#fff}
.nav-actions{display:flex;gap:10px;align-items:center}
.nav-btn{padding:8px 20px;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;transition:all 0.3s;text-decoration:none}
.nav-btn-outline{border:1px solid rgba(255,255,255,0.2);color:rgba(255,255,255,0.8);background:transparent}
.nav-btn-outline:hover{border-color:#818cf8;color:#818cf8;background:rgba(129,140,248,0.08)}
.nav-btn-solid{background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;border:none}
...
.hero-badge{display:inline-block;padding:6px 18px;border-radius:12px;background:rgba(99,102,241,0.15);border:1px solid rgba(99,102,241,0.25);color:#a5b4fc;font-size:13px;font-weight:500;margin-bottom:24px}
...
.hero-btn-primary{background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;box-shadow:0 8px 30px rgba(99,102,241,0.3)}
.hero-btn-primary:hover{transform:translateY(-2px);box-shadow:0 12px 40px rgba(99,102,241,0.4)}
.hero-btn-secondary{background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.8);border:1px solid rgba(255,255,255,0.12)}
...
.feature-card{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:12px;padding:28px 24px;transition:all 0.3s}
.feature-card:hover{background:rgba(255,255,255,0.07);border-color:rgba(255,255,255,0.12);transform:translateY(-4px)}
.feature-icon{width:48px;height:48px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:22px;margin-bottom:16px}
...
.step-num{width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,#6366f1,#a78bfa);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;font-size:18px;font-weight:800;color:#fff}
...
.cta-box{background:linear-gradient(135deg,rgba(99,102,241,0.1),rgba(168,85,247,0.08));border:1px solid rgba(99,102,241,0.2);border-radius:12px;padding:48px 32px;max-width:600px;margin:0 auto}
...
.footer{border-top:1px solid rgba(255,255,255,0.06);padding:24px 0;text-align:center;font-size:12px;color:rgba(255,255,255,0.25)}
...
@media(max-width:480px){
  .hero{padding:50px 0 40px}.hero h1{font-size:26px}
...
<div class="wrap">
  ...
  <section class="features" id="features">
    ...
    <div class="feature-grid">
      <div class="feature-card"><div class="feature-icon" style="background:rgba(99,102,241,0.15);color:#818cf8"><i class="fas fa-link"></i></div><h3><i class="fas fa-link"></i> 链接模块</h3><p>添加网页链接，支持自定义图标、卡片颜色、边框样式，还可设置访问密码保护隐私。</p></div>
      <div class="feature-card"><div class="feature-icon" style="background:rgba(34,197,94,0.15);color:#4ade80"><i class="fas fa-font"></i></div><h3><i class="fas fa-edit"></i> 文字模块</h3><p>展示纯文字内容，支持居中显示，点击弹出大图预览，适合展示公告、说明等信息。</p></div>
      <div class="feature-card"><div class="feature-icon" style="background:rgba(251,146,60,0.15);color:#fb923c"><i class="fas fa-image"></i></div><h3><i class="fas fa-image"></i> 图片模块</h3><p>在主页直接展示图片，点击可查看大图，适合作品展示、摄影集等场景。</p></div>
      <div class="feature-card"><div class="feature-icon" style="background:rgba(236,72,153,0.15);color:#f472b6"><i class="fas fa-images"></i></div><h3><i class="fas fa-image"></i> 弹窗图模块</h3><p>点击卡片弹出图片大图预览，适合展示海报、二维码、证书等需要放大的内容。</p></div>
      <div class="feature-card"><div class="feature-icon" style="background:rgba(168,85,247,0.15);color:#c084fc"><i class="fas fa-video"></i></div><h3><i class="fas fa-video"></i> 视频模块</h3><p>嵌入视频链接，弹窗播放，支持设置封面图和循环播放，适配移动端体验。</p></div>
      <div class="feature-card"><div class="feature-icon" style="background:rgba(14,165,233,0.15);color:#38bdf8"><i class="fas fa-palette"></i></div><h3><i class="fas fa-palette"></i> 个性化装扮</h3><p>自定义背景、卡片样式、主题模式，打造独一无二的个人主页风格。</p></div>
      <div class="feature-card"><div class="feature-icon" style="background:rgba(234,179,8,0.15);color:#eab308"><i class="fas fa-music"></i></div><h3><i class="fas fa-music"></i> 背景音乐</h3><p>添加自定义音乐链接，支持循环播放、自动播放，让主页更有氛围感。</p></div>
      <div class="feature-card"><div class="feature-icon" style="background:rgba(239,68,68,0.15);color:#f87171"><i class="fas fa-share-alt"></i></div><h3><i class="fas fa-link"></i> 社交链接</h3><p>集成微信、QQ、Telegram、抖音、B站、小红书、微博、GitHub、邮箱等社交渠道。</p></div>
      <div class="feature-card"><div class="feature-icon" style="background:rgba(99,102,241,0.15);color:#818cf8"><i class="fas fa-chart-simple"></i></div><h3><i class="fas fa-chart-bar"></i> 数据统计</h3><p>实时统计主页访问量和链接点击量，了解你的主页受欢迎程度。</p></div>
    </div>
  </section>
