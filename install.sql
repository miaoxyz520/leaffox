...
CREATE TABLE IF NOT EXISTS `page_likes` (
  ...
  `page_user_id` int(10) UNSIGNED NOT NULL COMMENT '被点赞的主页主人用户ID',
  `visitor_id` int(10) UNSIGNED NOT NULL COMMENT '点赞者用户ID',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_like` (`page_user_id`,`visitor_id`),
  KEY `idx_likes_owner` (`page_user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `page_comments` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `page_user_id` int(10) UNSIGNED NOT NULL COMMENT '被评论的主页主人用户ID',
  `visitor_id` int(10) UNSIGNED NOT NULL COMMENT '评论者用户ID',
  `content` varchar(500) NOT NULL COMMENT '评论内容',
  `status` tinyint(1) DEFAULT 1 COMMENT '0=待审核 1=已通过(公开) 2=隐藏',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`),
  KEY `idx_comments_page` (`page_user_id`,`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `page_favorites` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `page_user_id` int(10) UNSIGNED NOT NULL COMMENT '被收藏的主页主人用户ID',
  `visitor_id` int(10) UNSIGNED NOT NULL COMMENT '收藏者用户ID',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_fav` (`page_user_id`,`visitor_id`),
  KEY `idx_favs_owner` (`page_user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `enable_likes` tinyint(1) NOT NULL DEFAULT 1;
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `enable_comments` tinyint(1) NOT NULL DEFAULT 1;
ALTER TABLE `users` ADD COLUMN IF NOT EXISTS `enable_favorites` tinyint(1) NOT NULL DEFAULT 1;
