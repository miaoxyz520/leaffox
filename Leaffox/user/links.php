...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'add' || $action === 'edit') {
        ...
        $icon       = safeSubstr(trim($_POST['icon'] ?? '🔗'), 100);
...
$typeLabels = ['link'=>'🔗 链接','text'=>'📝 文字','image'=>'🖼️ 弹图','picture'=>'🖼️ 图片','video'=>'🎬 视频'];
...
<h1 class="text-xl font-bold mb-2">📦 模块管理</h1>
...
<div class="flex gap-2 mb-5 flex-wrap">
  <button onclick="openModal('add','link')" class="btn-sm btn-primary"><i class="fas fa-plus"></i> 🔗 链接</button>
  <button onclick="openModal('add','text')" class="btn-sm btn-ghost"><i class="fas fa-plus"></i> 📝 文字</button>
  <button onclick="openModal('add','image')" class="btn-sm btn-ghost"><i class="fas fa-plus"></i> 🖼️ 弹图</button>
  <button onclick="openModal('add','picture')" class="btn-sm btn-ghost"><i class="fas fa-plus"></i> 🖼️ 图片</button>
  <button onclick="openModal('add','video')" class="btn-sm btn-ghost"><i class="fas fa-plus"></i> 🎬 视频</button>
</div>
  ...
  <div class="card-base flex items-center gap-3 py-3 px-4 <?=$l['is_hidden']?'opacity-50':''?>">
    ...
    <div class="flex-1 min-w-0">
      <div class="text-white font-medium text-sm truncate flex items-center gap-2">
        ...
        <?php if(!empty($l['passcode'])):?><span class="text-xs text-yellow-400">🔒</span><?php endif;?>
      </div>
...
<div id="modalOverlay" class="modal-overlay" style="display:none" onclick="if(event.target===this)closeModal()">
  <div class="modal-box sm" onclick="event.stopPropagation()">
    <div class="flex justify-between items-center mb-4">
      ...
      <span onclick="closeModal()" class="cursor-pointer text-gray-500 hover:text-white text-xl">✕</span>
    </div>
    <form method="POST" id="modalForm" enctype="multipart/form-data">
      ...
      <div id="fieldCommon" class="space-y-3">
        ...
        <div id="iconField">
          ...
          <input type="text" name="icon" id="formIcon" placeholder="🔗">
        </div>
      </div>
      ...
      <div id="fieldVideo" class="space-y-3 hidden">
        <div><label class="block text-gray-300 text-xs font-medium mb-1.5">视频来源</label>
          <select name="video_source" id="formVideoSource" onchange="toggleVideoSource()" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white outline-none focus:border-indigo-500/50">
            <option value="file">📁 上传/直链</option>
        ...
        <div id="vsrcExternal" class="hidden">
          ...
          <p id="vsrcHint" class="text-xs text-gray-500 mt-1">💡 粘贴完整视频链接后，访问者点击将跳转至对应平台观看</p>
        </div>
      </div>
...
function toggleVideoSource(){
  ...
  if(hint){
    var labels = {'bilibili':'📺 跳转B站观看，支持iframe内嵌播放','douyin':'🎵 跳转抖音App或网页观看','kuaishou':'📱 跳转快手App或网页观看'};
    hint.textContent = '💡 ' + (labels[vs.value] || '粘贴完整视频链接后，访问者点击将跳转至对应平台观看');
  }
}
