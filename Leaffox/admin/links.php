...
$typeLabels = ['link'=>'🔗链接','text'=>'📝文字','image'=>'🖼️弹图','picture'=>'🖼️图片','video'=>'🎬视频'];
?>
...
<tr class="border-b border-white/5 hover:bg-white/5 transition">
  ...
  <td class="p-4 max-w-[220px]">
    ...
    <?php else: ?>
      <div class="flex items-center gap-2">
        <span class="text-lg"><?=h(mb_substr($l['icon']?:'🔗',0,2))?></span>
  ...
  <td class="p-4 whitespace-nowrap">
    ...
    <?php if(!empty($l['passcode'])):?><span class="text-yellow-400 bg-yellow-500/10 px-2 py-0.5 rounded text-xs">🔒</span><?php endif;?>
  </td>
...
function previewItem(url, type) {
  ...
  if (type === 'video') {
    ...
    box.innerHTML = '<img src="' + url.replace(/'/g,"%27") + '" style="max-width:85vw;max-height:85vh;border-radius:12px;box-shadow:0 20px 60px rgba(0,0,0,0.5);object-fit:contain;" onerror="this.outerHTML=\'<div style=\\\'color:#999;text-align:center;padding:40px;\\\'><div style=\\\'font-size:48px;margin-bottom:16px;\\\'>❌</div><p>图片加载失败</p></div>\'">';
  }
