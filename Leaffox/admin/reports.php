      ...
      <tr class="border-b border-white/5 hover:bg-white/[0.02]">
        ...
        <td class="py-3">
          <form method="POST" class="flex items-center gap-1" onsubmit="return confirm('确认操作？')">
            ...
            <?php if ($r['status']): ?>
              <button type="submit" name="action" value="unresolve" class="px-2 py-1 text-xs rounded-lg bg-white/5 text-gray-400 hover:text-amber-300 hover:bg-amber-500/10 transition" title="标记为未处理">🔁 重开</button>
            <?php else: ?>
              <button type="submit" name="action" value="resolve" class="px-2 py-1 text-xs rounded-lg bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition" title="标记为已处理">✅ 处理</button>
            <?php endif; ?>
            <button type="submit" name="action" value="delete" class="px-2 py-1 text-xs rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 transition" title="删除">🗑️</button>
          </form>
        </td>
      </tr>
      <?php endforeach; ?>
